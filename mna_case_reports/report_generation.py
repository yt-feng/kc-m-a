...
def improve_heading_text(body: str, *, is_last: bool = False) -> str:
    """Turn outline-like headings into objective, neutral section headings.

    The headings should read like a factual report, not a prompt outline and not
    a slogan.  They should still signal the issue a reader can learn from.
    """
...
def build_prompt(brief: CaseBrief, research_rows: list[dict[str, str]], *, revision_issues: list[str] | None = None, previous_article: dict[str, object] | None = None, expansion_only: bool = False) -> list[dict[str, str]]:
    ...
    if expansion_only and previous_article:
        revise_text = (
            ...
            "不要输出读者对象，不要写'上市公司CEO/董事长/读者'等提示语。章节标题要客观中性，不使用口号化、负面化或广告化表述。"
    ...
    return [
        ...
        },
        {
            ...
            "content": (
                ...
                "\n【硬性要求】章节标题要客观、中性、克制，用一句话概括该章事实和关注点；不要口号化，不要负面化，不要广告化。不要把'交易动机、交易背景、交易结构设计、并购战略考量、标的筛选、并购后整合、价值释放'等分析提纲直接写进标题。"
