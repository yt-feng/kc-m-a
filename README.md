...
当巨潮公告采集为空时，自动使用东方财富的公开上市公司公告列表补充发现，完整翻页后按并购关键词和本周时间区间筛选，并保留公告原链接。遇到 401/403 会停止重复请求同一巨潮接口，继续独立信息源；备用来源分页不完整会明确报错，不把局部结果当作完整采集。

每个周区间在模型处理前保存原始候选和诊断记录。GitHub Actions 无论成功或失败都会上传本轮诊断 artifact，便于复核候选、失败阶段和最终校验结果；零案例和无效链接仍然不能通过校验。
