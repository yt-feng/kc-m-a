...
from .config import CASE_DISCOVERY_QUERIES, CATEGORIES, CLASSIC_CASE_SEEDS, DOMESTIC_CATEGORY_HINTS
...
def is_domestic_region(region: str) -> bool:
    text = region or ""
    return any(token in text for token in ("中国", "A股", "港股", "境内", "香港"))
...
def summarize_raw_items(raw_items: list[RawItem], target_count: int) -> list[CaseBrief]:
    if not raw_items:
        return []
    sample = [item.as_dict() for item in raw_items[: min(len(raw_items), 160)]]
    messages = [
        {"role": "system", "content": "你是并购案例研究选题编辑。只输出 JSON。"},
        {
            ...
            "content": (
                ...
                "要求：优先中国案例；交易主体、交易事件和启示维度要清楚；剔除纯传闻、纯政策、纯市场评论。"
                f"最多输出 {target_count} 个。分类只能用：{json.dumps(CATEGORIES, ensure_ascii=False)}。"
                "输出格式：{\"cases\":[{\"case_name\":...,\"category\":...,\"region\":...,\"source_title\":...,\"source_url\":...,\"published_at\":...,\"why\":...,\"is_domestic\":true/false}]}。"
...
def discover_backfill_cases(target_count: int) -> list[CaseBrief]:
    # Query recent two-year deal flow plus classic seeds. The web-scale search itself is
    # proxied through the existing rich source collectors so the action stays API-key-light.
    raw: list[RawItem] = []
    end = datetime.now(BEIJING_TZ).replace(microsecond=0)
...
def choose_balanced(briefs: list[CaseBrief], *, count: int = 4, min_domestic: int = 2, report_root: Path) -> list[CaseBrief]:
    ...
    # SPAC is allowed to lag behind less aggressively.
    def score(b: CaseBrief) -> tuple[int, int, int]:
        domestic_bonus = 0 if b.is_domestic else 1
        classic_penalty = 1 if b.is_classic else 0
        category_count = counts[b.category] + (2 if b.category == "SPAC" else 0)
        return (category_count, domestic_bonus, classic_penalty)
    ...
    if domestic_now < min_domestic:
        for brief in sorted([b for b in deduped if b.is_domestic and b.key() not in {s.key() for s in selected}], key=score):
            if domestic_now >= min_domestic:
                break
            if len(selected) < count:
                selected.append(brief)
            else:
                # Replace the least useful non-domestic case.
